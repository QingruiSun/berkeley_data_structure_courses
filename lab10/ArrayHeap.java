...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int ptr = index;
        while (ptr > 1) {
            int parentPtr = parentIndex(ptr);
            if (getNode(parentPtr).myPriority <= getNode(ptr).myPriority) {
                break;
            } else {
                swap(ptr, parentPtr);
                ptr = parentPtr;
    ...
    private void sink(int index) {
        // Throws an exception if index is invalid. DON'T CHANGE THIS LINE.
        validateSinkSwimArg(index);
        int nowLeftIndex = leftIndex(index);
        int nowRightIndex = rightIndex(index);
        int smallestIndex = index;
        if ((nowLeftIndex <= this.size) && (getNode(nowLeftIndex).myPriority < getNode(smallestIndex).myPriority)) {
            smallestIndex = nowLeftIndex;
        }
        if ((nowRightIndex <= this.size) && (getNode(nowRightIndex).myPriority < getNode(smallestIndex).myPriority)) {
            smallestIndex = nowRightIndex;
        }
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size; ++i) {
            if (getNode(i).item().equals(item)) {
                double prePriority = getNode(i).priority();
